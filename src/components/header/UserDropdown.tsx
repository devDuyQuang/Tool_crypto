...
export default function UserDropdown() {
  ...
  function handleSignOut() {
    closeDropdown();
    logout({ redirectTo: "/signin" });
  }

  const { displayName, email, avatarSrc, showEmail } = useMemo(() => {
    const fullName = (me?.fullName || "").trim();
    const email = (me?.email || "").trim();

    const displayName = fullName || email || "User";

    // ✅ FIX: nếu displayName == email thì KHÔNG show dòng email nữa
    const showEmail = !!email && displayName !== email;

    const avatarSrc = me?.avatarUrl || "/images/user/owner.jpg";

    return { displayName, email, avatarSrc, showEmail };
  }, [me]);
    ...
    <div className="relative">
      ...
      >
        <span className="mr-3 overflow-hidden rounded-full h-11 w-11">
          <Image width={44} height={44} src={avatarSrc} alt="User" />
        </span>
        ...
        <ul className="flex flex-col gap-1 pt-4 pb-3 border-b border-gray-200 dark:border-gray-800">
          ...
          <li>
            ...
            >
              {/* ICON user/profile */}
              <svg
                className="shrink-0 fill-gray-500 group-hover:fill-gray-700 dark:fill-gray-400 dark:group-hover:fill-gray-300"
                width="22"
                height="22"
                viewBox="0 0 24 24"
                fill="none"
