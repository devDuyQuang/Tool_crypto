...
export default function BotProfileEditPage() {
    ...
    useEffect(() => {
        botProfilesService.findOne(id)
            .then((loaded) => {
                ...
                setContextTimeframes((loaded.contextTimeframes?.length ? loaded.contextTimeframes : ["H1", "M30", "M15"]).join(","));
                setTriggerTimeframe(loaded.triggerTimeframe || "M5");
                setRiskPerTradePercent(loaded.riskPerTradePercent ?? officialPolicy.riskPerTradePercent);
                setDailyLossLimitPercent(loaded.dailyLossLimitPercent ?? officialPolicy.dailyLossLimitPercent);
                setMaxConcurrentPositions(loaded.maxConcurrentPositions ?? officialPolicy.maxConcurrentPositions);
                setMaxPositionsPerSymbol(loaded.maxPositionsPerSymbol ?? officialPolicy.maxPositionsPerSymbol);
                setMaxTradesPerHour(loaded.maxTradesPerHour ?? officialPolicy.maxTradesPerHour);
                setMaxLeverage(loaded.maxLeverage ?? officialPolicy.maxLeverage);
                setMaxMarginPerTradeUsdt(loaded.maxMarginPerTradeUsdt ?? officialPolicy.maxMarginPerTradeUsdt);
                setMaxNotionalPerTradeUsdt(loaded.maxNotionalPerTradeUsdt ?? officialPolicy.maxNotionalPerTradeUsdt);
                setMaxLossPerTradeUsdt(loaded.maxLossPerTradeUsdt ?? officialPolicy.maxLossPerTradeUsdt);
    ...
    const validationError = useMemo(() => {
        ...
        return null;
    }, [name, contextTimeframes, triggerTimeframe, riskPerTradePercent, dailyLossLimitPercent, maxConcurrentPositions, maxPositionsPerSymbol, maxTradesPerHour, maxLeverage, maxMarginPerTradeUsdt, maxNotionalPerTradeUsdt, maxLossPerTradeUsdt]);
        ...
        <div className="space-y-6">
            ...
            <div className="flex justify-between">
                ...
                {step < steps.length - 1 ? (
                    ...
                    <button disabled={saving} onClick={save} className="rounded-lg bg-brand-600 px-4 py-2 text-sm font-medium text-white disabled:opacity-50">
                        Lưu thay đổi
                    </button>
                )}
            </div>
